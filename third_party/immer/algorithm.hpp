...
void for_each_chunk(const Iterator& first, const Iterator& last, Fn&& fn) {
    ...
    first.impl().for_each_chunk(first.index(), last.index(), std::forward<Fn>(fn));
}
...
bool for_each_chunk_p(const Iterator& first, const Iterator& last, Fn&& fn) {
    assert(&first.impl() == &last.impl());
    return first.impl().for_each_chunk_p(first.index(), last.index(), std::forward<Fn>(fn));
}
...
T accumulate(Range&& r, T init) {
    for_each_chunk(r, [&](auto first, auto last) { init = std::accumulate(first, last, init); });
    return init;
}
...
T accumulate(Range&& r, T init, Fn fn) {
    for_each_chunk(r,
                   [&](auto first, auto last) { init = std::accumulate(first, last, init, fn); });
    return init;
}
...
T accumulate(Iterator first, Iterator last, T init) {
    for_each_chunk(first, last,
                   [&](auto first, auto last) { init = std::accumulate(first, last, init); });
    return init;
}
...
OutIter copy(Range&& r, OutIter out) {
    for_each_chunk(r, [&](auto first, auto last) { out = std::copy(first, last, out); });
    return out;
}
...
OutIter copy(InIter first, InIter last, OutIter out) {
    for_each_chunk(first, last, [&](auto first, auto last) { out = std::copy(first, last, out); });
    return out;
}
...
bool all_of(Range&& r, Pred p) {
    return for_each_chunk_p(r, [&](auto first, auto last) { return std::all_of(first, last, p); });
}
...
bool all_of(Iter first, Iter last, Pred p) {
    return for_each_chunk_p(first, last,
                            [&](auto first, auto last) { return std::all_of(first, last, p); });
}
