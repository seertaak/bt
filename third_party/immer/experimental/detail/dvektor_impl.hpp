...
namespace immer { namespace detail { namespace dvektor {
    ...
    template <int B, typename T = std::size_t>
    constexpr auto max_depth = fast_log2(std::numeric_limits<std::size_t>::max()) / B;
    ...
    struct node : enable_intrusive_ptr<node<T, B, MP>, typename MP::refcount>,
        ...
        ~node() {
            switch (kind) {
            case leaf_kind: data.leaf.~leaf_node_t(); break;
            case inner_kind: data.inner.~inner_node_t(); break;
            }
        }
    ...
    struct ref {
        ...
        node_ptr_t copy_of_inner(const node_ptr_t& n) { return make_node(n->inner()); }

        node_ptr_t copy_of_leaf(const node_ptr_t& n) { return make_node(n->leaf()); }
        ...
        void goto_pos_writable_from_clean(std::size_t old_index,
            ...
            if (d == 0) {
                ...
                while (--d) {
                    display[d] =
                        null_slot_and_copy_inner(display[d + 1], (index >> shift) & mask<B>);
                    shift -= B;
                }
                display[0] = null_slot_and_copy_leaf(display[1], (index >> B) & mask<B>);
            }
        }
        ...
        void goto_pos_writable_from_dirty(std::size_t old_index,
            ...
            if (xr < (1 << B)) {
                ...
                for (auto i = 1u; i <= display_idx; ++i) {
                    display[i] = copy_of_inner(display[i]);
                    display[i]->inner()[(old_index >> shift) & mask<B>] = display[i - 1];
                    shift += B;
                }
                for (auto i = display_idx - 1; i > 0; --i) {
                    shift -= B;
                    display[i] =
                        null_slot_and_copy_inner(display[i + 1], (new_index >> shift) & mask<B>);
                }
                display[0] = null_slot_and_copy_leaf(display[1], (new_index >> B) & mask<B>);
            }
        }
        ...
        void goto_fresh_pos_writable_from_clean(std::size_t old_index,
            ...
            if (display_idx > 0) {
                ...
                while (--display_idx) {
                    auto node = display[display_idx + 1]->inner()[(new_index >> shift) & mask<B>];
                    display[display_idx] = node ? std::move(node) : make_node(inner_t{});
                }
        ...
        void goto_next_block_start(std::size_t index, std::size_t xr) {
            ...
            if (display_idx > 0) {
                ...
                while (--display_idx) display[display_idx - 1] = display[display_idx]->inner()[0];
            }
        }
    ...
    struct impl {
        ...
        impl push_back(T value) const {
            if (size) {
                ...
                return impl{1, 0, false, {1, {{make_node(leaf_t{{std::move(value)}})}}}};
            }
        }

        const T& get(std::size_t index) const { return p.get_elem(index, index ^ focus); }
    ...
    struct iterator
        : boost::
              iterator_facade<iterator<T, B, MP>, T, boost::random_access_traversal_tag, const T&> {
        struct end_t {};
...
}}}  // namespace immer::detail::dvektor
