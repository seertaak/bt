...
class array {
    using impl_t = std::conditional_t<MemoryPolicy::use_transient_rvalues,
...
public:
     ...
     */
    array(std::initializer_list<T> values) : impl_{impl_t::from_initializer_list(values)} {}
     ...
     */
    IMMER_NODISCARD bool operator==(const array& other) const { return impl_.equals(other.impl_); }
    IMMER_NODISCARD bool operator!=(const array& other) const { return !(*this == other); }
     ...
     * @endrst
     */
    IMMER_NODISCARD array take(size_type elems) const& { return impl_.take(elems); }
...
private:
    ...
    array take_move(std::false_type, size_type elems) { return impl_.take(elems); }
