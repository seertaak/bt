...
struct debug_size_heap {
    ...
    // It causes different versions of std::max_align_t to be defined, depending on inclusion order
    // of stddef.h and stdint.h. As we have no control over the inclusion order here (as it might be
    // set in stone by the outside world), we can't easily pin it to one of both versions of
    // std::max_align_t. This means, we have to hardcode extra_size for MinGW 32bit builds until the
    // mentioned bug is fixed.
    constexpr static auto extra_size = 8;
#else
    constexpr static auto extra_size =
        sizeof(std::aligned_storage_t<sizeof(std::size_t), alignof(std::max_align_t)>);
#endif
