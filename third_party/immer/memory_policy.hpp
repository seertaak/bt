...
struct get_prefer_fewer_bigger_objects
    : std::integral_constant<bool, std::is_same<HeapPolicy, heap_policy<cpp_heap>>::value> {};

template <typename T>
constexpr auto get_prefer_fewer_bigger_objects_v = get_prefer_fewer_bigger_objects<T>::value;
...
struct get_use_transient_rvalues
    : std::integral_constant<bool, !std::is_same<RefcountPolicy, no_refcount_policy>::value> {};
