...
class map {
    ...
    struct project_value {
        const T& operator()(const value_t& v) const noexcept { return v.second; }
    };
    ...
    struct error_value {
        const T& operator()() const { throw std::out_of_range{"key not found"}; }
    };
    ...
    struct equal_key {
        auto operator()(const value_t& a, const value_t& b) { return Equal{}(a.first, b.first); }

        auto operator()(const value_t& a, const K& b) { return Equal{}(a.first, b); }
    };
...
public:
    ...
    IMMER_NODISCARD size_type count(const K& k) const {
        return impl_
            .template get<detail::constantly<size_type, 1>, detail::constantly<size_type, 0>>(k);
    }
     ...
     * complexity is *effectively* @f$ O(1) @f$.
     */
    const T& at(const K& k) const { return impl_.template get<project_value, error_value>(k); }
    ...
    IMMER_NODISCARD const T* find(const K& k) const {
        return impl_.template get<project_value_ptr, detail::constantly<const T*, nullptr>>(k);
    }
    ...
    IMMER_NODISCARD bool operator!=(const map& other) const { return !(*this == other); }
     ...
     * O(1) @f$.
     */
    IMMER_NODISCARD map insert(value_type value) const { return impl_.add(std::move(value)); }
    ...
    IMMER_NODISCARD map update(key_type k, Fn&& fn) const {
        return impl_.template update<project_value, default_value, combine_value>(
