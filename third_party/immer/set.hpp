...
public:
    ...
    IMMER_NODISCARD size_type count(const T& value) const {
        return impl_
            .template get<detail::constantly<size_type, 1>, detail::constantly<size_type, 0>>(
                value);
    }
     ...
     */
    IMMER_NODISCARD bool operator==(const set& other) const { return impl_.equals(other.impl_); }
    IMMER_NODISCARD bool operator!=(const set& other) const { return !(*this == other); }
     ...
     */
    IMMER_NODISCARD set insert(T value) const { return impl_.add(std::move(value)); }
