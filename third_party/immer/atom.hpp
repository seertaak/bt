...
namespace detail {
    ...
    struct refcount_atom_impl {
        ...
        refcount_atom_impl(box_type b) : impl_{std::move(b)} {}
    ...
    struct gc_atom_impl {
        ...
        static_assert(std::is_same<typename MemoryPolicy::refcount, no_refcount_policy>::value,
        ...
        gc_atom_impl(box_type b) : impl_{b.impl_} {}
        ...
        box_type update(Fn&& fn) {
            while (true) {
                ...
                auto newv = oldv.update(fn);
                if (impl_.compare_exchange_weak(oldv.impl_, newv.impl_)) return {newv};
            }
        }
...
public:
     ...
     */
    atom(box_type v = {}) : impl_{std::move(v)} {}
     ...
     */
    IMMER_NODISCARD box_type exchange(box_type b) { return impl_.exchange(std::move(b)); }
