...
template <typename T, typename MP, detail::rbts::bits_t B, detail::rbts::bits_t BL>
class vector;
...
public:
     ...
     */
    flex_vector(std::initializer_list<T> values) : impl_{impl_t::from_initializer_list(values)} {}
    ...
    template <typename Iter,
              typename Sent,
              std::enable_if_t<detail::compatible_sentinel_v<Iter, Sent>, bool> = true>
    flex_vector(Iter first, Sent last) : impl_{impl_t::from_range(first, last)} {}
    ...
    flex_vector(vector<T, MemoryPolicy, B, BL> v)
        : impl_{v.impl_.size, v.impl_.shift, v.impl_.root->inc(), v.impl_.tail->inc()} {}
     ...
     * @f$.
     */
    IMMER_NODISCARD reference operator[](size_type index) const { return impl_.get(index); }
    ...
    IMMER_NODISCARD bool operator!=(const flex_vector& other) const { return !(*this == other); }
     ...
     * @endrst
     */
    IMMER_NODISCARD flex_vector take(size_type elems) const& { return impl_.take(elems); }

    IMMER_NODISCARD decltype(auto) take(size_type elems) && { return take_move(move_t{}, elems); }
     ...
     * @endrst
     */
    IMMER_NODISCARD flex_vector drop(size_type elems) const& { return impl_.drop(elems); }

    IMMER_NODISCARD decltype(auto) drop(size_type elems) && { return drop_move(move_t{}, elems); }
    ...
    IMMER_NODISCARD decltype(auto) insert(size_type pos, T value) && {
        using std::move;
        auto rs = drop(pos);
        return std::move(*this).take(pos).push_back(std::move(value)) + std::move(rs);
    }
     ...
     * @endrst
     */
    IMMER_NODISCARD flex_vector erase(size_type pos) const& { return take(pos) + drop(pos + 1); }
     ...
     */
    IMMER_NODISCARD transient_type transient() const& { return transient_type{impl_}; }
    IMMER_NODISCARD transient_type transient() && { return transient_type{std::move(impl_)}; }

    // Semi-private
    const impl_t& impl() const { return impl_; }

#if IMMER_DEBUG_PRINT
    void debug_print(std::ostream& out = std::cerr) const { impl_.debug_print(out); }
#endif
...
private:
    ...
    flex_vector take_move(std::false_type, size_type elems) { return impl_.take(elems); }

    flex_vector&& drop_move(std::true_type, size_type elems) {
        impl_.drop_mut({}, elems);
        return std::move(*this);
    }
    flex_vector drop_move(std::false_type, size_type elems) { return impl_.drop(elems); }
