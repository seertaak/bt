...
namespace immer { namespace detail { namespace rbts {
    ...
    struct node {
        ...
        using relaxed_t =
            std::conditional_t<embed_relaxed, relaxed_data_no_meta_t, relaxed_data_with_meta_t>;
        ...
        constexpr static std::size_t sizeof_packed_leaf_n(count_t count) {
            return immer_offsetof(impl_t, d.data.leaf.buffer) + sizeof(leaf_t::buffer) * count;
        }
        ...
        constexpr static std::size_t sizeof_packed_inner_r_n(count_t count) {
            return embed_relaxed ? sizeof_packed_inner_n(count) + sizeof_packed_relaxed_n(count)
                                 : sizeof_packed_inner_n(count);
        }

        constexpr static std::size_t max_sizeof_leaf = sizeof_packed_leaf_n(branches<BL>);

        constexpr static std::size_t max_sizeof_inner = sizeof_packed_inner_n(branches<B>);

        constexpr static std::size_t max_sizeof_relaxed = sizeof_packed_relaxed_n(branches<B>);

        constexpr static std::size_t max_sizeof_inner_r = sizeof_packed_inner_r_n(branches<B>);
        ...
        static node_t* make_inner_r_n(
            count_t n, node_t* x, size_t xs, node_t* y, size_t ys, node_t* z, size_t zs) {
            assert(n >= 3);
            auto p = make_inner_r_n(n);
            auto r = p->relaxed();
            p->inner()[0] = x;
        ...
        static node_t* copy_leaf(node_t* src1, count_t n1, node_t* src2, count_t n2) {
            ...
            try {
                std::uninitialized_copy(src1->leaf(), src1->leaf() + n1, dst->leaf());
            } catch (...) {
                heap::deallocate(node_t::sizeof_leaf_n(n1 + n2), dst);
                throw;
            }
            try {
                std::uninitialized_copy(src2->leaf(), src2->leaf() + n2, dst->leaf() + n1);
            } catch (...) {
        ...
        static node_t* copy_leaf_e(edit_t e, node_t* src, count_t idx, count_t last) {
            ...
            auto dst = make_leaf_e(e);
            try {
                std::uninitialized_copy(src->leaf() + idx, src->leaf() + last, dst->leaf());
            } catch (...) {
                heap::deallocate(max_sizeof_leaf, dst);
                throw;
            }
        ...
        static void delete_inner(node_t* p, count_t n) {
            ...
            assert(!p->relaxed());
            heap::deallocate(
                ownee(p).owned() ? node_t::max_sizeof_inner : node_t::sizeof_inner_n(n), p);
        }
        ...
        static void delete_inner_r(node_t* p, count_t n) {
            ...
            static_if<!embed_relaxed>([&](auto) {
                if (node_t::refs(r).dec())
                    heap::deallocate(node_t::ownee(r).owned() ? node_t::max_sizeof_relaxed
                                                              : node_t::sizeof_relaxed_n(n),
                                     r);
            });
            heap::deallocate(
                ownee(p).owned() ? node_t::max_sizeof_inner_r : node_t::sizeof_inner_r_n(n), p);
        }
        ...
        static void delete_inner_r_e(node_t* p) {
            ...
            assert(r);
            static_if<!embed_relaxed>([&](auto) {
                if (node_t::refs(r).dec()) heap::deallocate(node_t::max_sizeof_relaxed, r);
            });
        ...
        static void delete_leaf(node_t* p, count_t n) {
            ...
            destroy_n(p->leaf(), n);
            heap::deallocate(ownee(p).owned() ? node_t::max_sizeof_leaf : node_t::sizeof_leaf_n(n),
                             p);
        }

        bool can_mutate(edit_t e) const { return refs(this).unique() || ownee(this).can_mutate(e); }
        ...
        relaxed_t* ensure_mutable_relaxed(edit_t e) {
            ...
            return static_if<embed_relaxed, relaxed_t*>(
                [&](auto) { return src_r; },
                [&](auto) {
                    if (node_t::refs(src_r).unique() || node_t::ownee(src_r).can_mutate(e))
                        return src_r;
                    else {
        ...
        relaxed_t* ensure_mutable_relaxed_e(edit_t e, edit_t ec) {
            ...
            return static_if<embed_relaxed, relaxed_t*>(
                ...
                [&](auto) {
                    if (src_r &&
                        (node_t::refs(src_r).unique() || node_t::ownee(src_r).can_mutate(e))) {
            ...
            return static_if<embed_relaxed, relaxed_t*>(
                ...
                [&](auto) {
                    if (node_t::refs(src_r).unique() || node_t::ownee(src_r).can_mutate(e))
                        ...
                        if (src_r) node_t::refs(src_r).dec_unsafe();
                        auto dst_r = new (heap::allocate(max_sizeof_relaxed)) relaxed_t;
                        std::copy(src_r->d.sizes, src_r->d.sizes + n, dst_r->d.sizes);
                        node_t::ownee(dst_r) = e;
        ...
        bool check(shift_t shift, size_t size) {
            ...
            if (shift == endshift<B, BL>) {
                ...
                for (auto i = 1; i < count; ++i) assert(r->d.sizes[i - 1] < r->d.sizes[i]);
                ...
                if (count) {
                    ...
                    assert(inner()[count - 1]->check(shift - B, size - ((count - 1) << shift)));
                }
            }
