...
namespace immer { namespace detail { namespace rbts {
    ...
    struct for_each_chunk_p_left_visitor : visitor_base<for_each_chunk_p_left_visitor> {
    ...
    struct for_each_chunk_p_right_visitor : visitor_base<for_each_chunk_p_right_visitor> {
    ...
    struct equals_visitor : visitor_base<equals_visitor> {
        ...
        struct rrb : visitor_base<rrb> {
            ...
            static bool visit_node(
                PosR&& posr, Iter&& first, Node* rootl, shift_t shiftl, size_t sizel) {
                assert(shiftl <= posr.shift());
                return shiftl == posr.shift()
                           ? visit_maybe_relaxed_sub(rootl, shiftl, sizel, this_t{}, posr, first,
                                                     size_t{})
        ...
        static bool visit_relaxed(PosL&& posl, PosR&& posr, Iter&& first, size_t idx) {
            ...
            for (; i < cl; ++i) {
                ...
                for (; j + 1 < cr && (sbr = posr.size_before(j)) < sbl; ++j)
                    ;
                auto res = sbl == sbr ? posr.nth_sub(j, this_aux_t{}, i, posl, first, idx + sbl)
        ...
        template <typename PosL, typename PosR, typename Iter>
        static std::enable_if_t<is_relaxed_v<PosR>, bool> visit_regular(PosL&& posl,
                                                                        PosR&& posr,
                                                                        Iter&& first,
                                                                        size_t idx) {
        ...
        static std::enable_if_t<!is_relaxed_v<PosR>, bool> visit_regular(PosL&& posl,
                                                                         ...
                                                                         Iter&& first,
                                                                         size_t idx) {
            return posl.count() >= posr.count() ? this_t::visit_regular(posl, posr.node())
        ...
        static bool visit_leaf(PosL&& posl, PosR&& posr, Iter&& first, size_t idx) {
            ...
            return std::equal(posl.node()->leaf(), posl.node()->leaf() + mp, posr.node()->leaf()) &&
                   std::equal(posl.node()->leaf() + mp, posl.node()->leaf() + posl.count(),
                              first + (idx + mp));
        }
        ...
        static bool visit_leaf(Pos&& pos, NodeT* other) {
            auto node = pos.node();
            return node == other ||
                   std::equal(node->leaf(), node->leaf() + pos.count(), other->leaf());
        }
    };
    ...
    struct update_visitor : visitor_base<update_visitor<NodeT>> {
        ...
        static node_t* visit_leaf(Pos&& pos, size_t idx, Fn&& fn) {
            ...
            try {
                node->leaf()[offset] = std::forward<Fn>(fn)(std::move(node->leaf()[offset]));
                return node;
            } catch (...) {
    ...
    struct get_mut_visitor : visitor_base<get_mut_visitor<NodeT>> {
        ...
        static value_t& visit_relaxed(Pos&& pos, size_t idx, edit_t e, node_t** location) {
            ...
            if (node->can_mutate(e)) {
                return pos.towards_oh(this_t{}, idx, offset, e, &node->inner()[offset]);
            } else {
                auto new_node = node_t::copy_inner_sr_e(e, node, count);
                try {
                    auto& res =
                        pos.towards_oh(this_t{}, idx, offset, e, &new_node->inner()[offset]);
        ...
        static value_t& visit_regular(Pos&& pos, size_t idx, edit_t e, node_t** location) {
            ...
            auto node = pos.node();
            if (node->can_mutate(e)) {
                return pos.towards_oh_ch(this_t{}, idx, offset, count, e, &node->inner()[offset]);
            } else {
    ...
    struct push_tail_mut_visitor : visitor_base<push_tail_mut_visitor<NodeT, Mutating>> {
        ...
        static node_t* visit_relaxed(Pos&& pos, edit_t e, node_t* tail, count_t ts) {
            ...
            auto new_idx = children == size_t{1} << level || level == BL ? idx + 1 : idx;
            ...
            if (new_idx >= branches<B>)
                ...
                new_child = mutate ? pos.last_oh_csh(this_t{}, idx, children, e, tail, ts)
        ...
        static node_t* visit_regular(Pos&& pos, edit_t e, node_t* tail, Args&&...) {
            ...
            auto mutate = Mutating && node->can_mutate(e);
            if (mutate) {
                node->inner()[new_idx] = idx == new_idx
                                             ? pos.last_oh(this_t{}, idx, e, tail)
                                             /* otherwise */
    ...
    struct push_tail_visitor : visitor_base<push_tail_visitor<NodeT>> {
        ...
        static node_t* visit_regular(Pos&& pos, node_t* tail, Args&&...) {
            ...
            try {
                new_parent->inner()[new_idx] = idx == new_idx
    ...
    struct slice_right_mut_visitor
        ...
        static result_t visit_relaxed(PosT&& pos, size_t last, edit_t e) {
            ...
            if (Collapse && idx == 0) {
                ...
                using std::get;
                auto subs = mutate ? pos.towards_oh(no_collapse_t{}, last, idx, e)
                ...
                try {
                    ...
                    if (next)
                        dec_inner(next, pos.shift() - B, last + 1 - ts - pos.size_before(idx));
                    dec_leaf(tail, ts);
                    throw;
                }
            }
        ...
        static result_t visit_leaf(PosT&& pos, size_t last, edit_t e) {
            ...
            if (new_tail_size == old_tail_size) {
                ...
                destroy_n(node->leaf() + new_tail_size, old_tail_size - new_tail_size);
                return std::make_tuple(0, nullptr, new_tail_size, node);
            } else {
    ...
    struct slice_left_mut_visitor
        ...
        static result_t visit_relaxed(PosT&& pos, size_t first, edit_t e) {
            ...
            if (Collapse && pos.shift() > BL && idx == pos.count() - 1) {
                ...
                using std::get;
                auto newn =
                    mutate ? (node->ensure_mutable_relaxed(e), node) : node_t::make_inner_r_e(e);
                auto newr = newn->relaxed();
                auto newcount = count - idx;
                auto new_child_size = child_size - child_dropped_size;
                try {
                    auto subs = mutate ? pos.towards_sub_oh(no_collapse_t{}, first, idx, e)
                    ...
                    std::uninitialized_copy(node->inner() + idx + 1, node->inner() + count,
                                            newn->inner() + 1);
        ...
        static result_t visit_regular(PosT&& pos, size_t first, edit_t e) {
            ...
            if (Collapse && pos.shift() > BL && idx == pos.count() - 1) {
                ...
                // `node<...>` static method...
                auto newcount = count - idx;
                auto newn = mutate ? (node->impl.d.data.inner.relaxed = new (node_t::heap::allocate(
                                          node_t::max_sizeof_relaxed, norefs_tag{})) relaxed_t,
                                      node)
        ...
        static result_t visit_leaf(PosT&& pos, size_t first, edit_t e) {
            ...
            auto count = pos.count();
            auto mutate = Mutating && std::is_nothrow_move_constructible<value_t>::value &&
                          node->can_mutate(e);
            if (mutate) {
                auto data = node->leaf();
                auto newcount = count - idx;
        ...
        static result_t visit_inner(PosT&& pos, size_t first) {
            ...
            if (Collapse && pos.shift() > BL && idx == pos.count() - 1) {
                ...
                try {
                    ...
                    newr->d.sizes[0] = child_size - child_dropped_size;
                    pos.copy_sizes(idx + 1, newr->d.count - 1, newr->d.sizes[0], newr->d.sizes + 1);
                    assert(newr->d.sizes[newr->d.count - 1] == pos.size() - dropped_size);
                    newn->inner()[0] = get<1>(subs);
                    std::uninitialized_copy(n->inner() + idx + 1, n->inner() + count,
                                            newn->inner() + 1);
        ...
        void merge_leaf(Pos&& p) {
            ...
            if (!to_ && *curr_ == from_count) {
                ...
                do {
                    ...
                    auto data = to_->leaf();
                    auto to_copy = std::min(from_count - from_offset, *curr_ - to_offset_);
                    std::uninitialized_copy(from_data + from_offset,
                                            from_data + from_offset + to_copy, data + to_offset_);
                    to_offset_ += to_copy;
                    from_offset += to_copy;
                    if (*curr_ == to_offset_) {
        ...
        void merge_inner(Pos&& p) {
            ...
            if (!to_ && *curr_ == from_count) {
                ...
                do {
                    ...
                    p.copy_sizes(from_offset, to_copy, to_size_, sizes + to_offset_);
                    to_offset_ += to_copy;
                    from_offset += to_copy;
    ...
    struct concat_rebalance_plan_fill_visitor : visitor_base<concat_rebalance_plan_fill_visitor> {
...
#pragma GCC diagnostic pop
    ...
    concat_center_pos<Node> concat_leafs(LPos&& lpos, TPos&& tpos, RPos&& rpos) {
        ...
        if (tpos.count() > 0)
            return {
                Node::bits_leaf, lpos.node()->inc(), lpos.count(), tpos.node()->inc(),
                tpos.count(),    rpos.node()->inc(), rpos.count(),
            };
        else
            return {
                Node::bits_leaf, lpos.node()->inc(), lpos.count(), rpos.node()->inc(), rpos.count(),
            };
    }
    ...
    template <typename Node>
    struct concat_trees_right_visitor : visitor_base<concat_trees_right_visitor<Node>> {
    ...
    struct concat_trees_left_visitor : visitor_base<concat_trees_left_visitor<Node>> {
        ...
        static concat_center_pos<Node> visit_node(LPos&& lpos, TPos&& tpos, Args&&... args) {
            return visit_maybe_relaxed_sub(args..., concat_trees_right_visitor<Node>{}, lpos, tpos);
        }
    };
    ...
    relaxed_pos<Node> concat_trees(Node* lroot,
                                   ...
                                   shift_t rshift,
                                   size_t rsize) {
        return visit_maybe_relaxed_sub(lroot, lshift, lsize, concat_trees_left_visitor<Node>{},
    ...
    relaxed_pos<Node> concat_trees(
        ...
        return make_singleton_regular_sub_pos(ltail, ltcount)
            .visit(concat_trees_left_visitor<Node>{}, empty_leaf_pos<Node>{}, rroot, rshift, rsize)
            .realize();
    }
                ...
                do {
                    ...
                    if (from == to_) {
                        if (from_offset != to_offset_)
                            std::move(from_data + from_offset, from_data + from_offset + to_copy,
                                      data + to_offset_);
                    } else {
                        if (!from_mutate)
                            std::uninitialized_copy(from_data + from_offset,
            ...
            if (!to_ && *curr_ == from_count) {
                ...
                do {
                    ...
                    if (from != to_ || from_offset != to_offset_) {
                        std::copy(from_data + from_offset, from_data + from_offset + to_copy,
                                  data + to_offset_);
                        p.copy_sizes(from_offset, to_copy, to_size_, sizes + to_offset_);
                    }
                    to_offset_ += to_copy;
    ...
    struct concat_rebalance_plan_mut : concat_rebalance_plan<B, BL> {
        ...
        concat_center_mut_pos<node_type<CPos>> merge(edit_type<CPos> ec,
            ...
            auto merger =
                merger_t{ec, cpos.shift(), this->counts, this->n, el, lmut2 ? lnode : nullptr};
            try {
    ...
    concat_center_mut_pos<Node> concat_leafs_mut(edit_type<Node> ec,
        ...
        if (tpos.count() > 0)
            ...
            return {
                Node::bits_leaf, lpos.node(), lpos.count(), rpos.node(), rpos.count(),
            };
    }
    ...
    concat_center_mut_pos<Node> concat_inners_mut(edit_type<Node> ec,
        ...
        if (lshift > rshift) {
            auto cpos = lpos.last_sub(concat_left_mut_visitor<Node>{}, ec, el, tpos, er, rpos);
            return concat_rebalance_mut<Node>(ec, el, lpos, cpos, er, null_sub_pos{});
        } else if (lshift < rshift) {
            auto cpos = rpos.first_sub(concat_right_mut_visitor<Node>{}, ec, el, lpos, tpos, er);
            return concat_rebalance_mut<Node>(ec, el, null_sub_pos{}, cpos, er, rpos);
        } else {
            assert(lshift == rshift);
            assert(Node::bits_leaf == 0u || lshift > 0);
            auto cpos = lpos.last_sub(concat_both_mut_visitor<Node>{}, ec, el, tpos, er, rpos);
    ...
    struct concat_both_mut_visitor : visitor_base<concat_both_mut_visitor<Node>> {
        ...
        static concat_center_mut_pos<Node> visit_inner(
            LPos&& lpos, edit_t ec, edit_t el, TPos&& tpos, edit_t er, RPos&& rpos) {
            return rpos.first_sub(concat_right_mut_visitor<Node>{}, ec, el, lpos, tpos, er);
        }

        template <typename LPos, typename TPos, typename RPos>
        static concat_center_mut_pos<Node> visit_leaf(
            LPos&& lpos, edit_t ec, edit_t el, TPos&& tpos, edit_t er, RPos&& rpos) {
            return rpos.first_sub_leaf(concat_right_mut_visitor<Node>{}, ec, el, lpos, tpos, er);
        }
    };

    template <typename Node>
    struct concat_trees_right_mut_visitor : visitor_base<concat_trees_right_mut_visitor<Node>> {
    ...
    template <typename Node>
    struct concat_trees_left_mut_visitor : visitor_base<concat_trees_left_mut_visitor<Node>> {
        using this_t = concat_trees_left_mut_visitor;
        using edit_t = typename Node::edit_t;

        template <typename LPos, typename TPos, typename... Args>
        static concat_center_mut_pos<Node> visit_node(
            LPos&& lpos, edit_t ec, edit_t el, TPos&& tpos, edit_t er, Args&&... args) {
            return visit_maybe_relaxed_sub(args..., concat_trees_right_mut_visitor<Node>{}, ec, el,
                                           lpos, tpos, er);
        }
    };
    ...
    relaxed_pos<Node> concat_trees_mut(edit_type<Node> ec,
                                       ...
                                       shift_t rshift,
                                       size_t rsize) {
        return visit_maybe_relaxed_sub(lroot, lshift, lsize, concat_trees_left_mut_visitor<Node>{},
                                       ec, el, make_leaf_pos(ltail, ltcount), er, rroot, rshift,
                                       rsize)
            .realize_e(ec);
    }
    ...
    relaxed_pos<Node> concat_trees_mut(edit_type<Node> ec,
                                       ...
                                       size_t rsize) {
        return make_singleton_regular_sub_pos(ltail, ltcount)
            .visit(concat_trees_left_mut_visitor<Node>{}, ec, el, empty_leaf_pos<Node>{}, er, rroot,
                   rshift, rsize)
            .realize_e(ec);
    }
