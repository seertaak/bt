...
namespace immer { namespace detail { namespace rbts {
    ...
    struct regular_pos {
        ...
        decltype(auto) towards_oh_ch(
            Visitor v, size_t idx, count_t offset_hint, count_t count_hint, Args&&... args) {
            return towards_oh_ch_regular(*this, v, idx, offset_hint, count(), args...);
        }
        ...
        if (p.shift() == BL) {
            ...
            if (l > f) {
                ...
                if (l < p.count()) {
                    ...
                    for (; n < e; ++n)
                        if (!make_full_pos(*n, ss).visit(v, args...)) return false;
                    if (!make_regular_pos(*n, ss, p.size()).visit(v, args...)) return false;
                }
            }
        }
        return true;
    ...
    template <typename Pos, typename Visitor, typename... Args>
    decltype(auto) towards_oh_ch_regular(
        Pos&& p, Visitor v, size_t idx, count_t offset_hint, count_t count_hint, Args&&... args) {
        constexpr auto B = bits<Pos>;
        constexpr auto BL = bits_leaf<Pos>;
        ...
        return is_full ? (is_leaf ? make_full_leaf_pos(child).visit(v, idx, args...)
                                  : make_full_pos(child, p.shift() - B).visit(v, idx, args...))
                       : (is_leaf ? make_leaf_pos(child, p.size()).visit(v, idx, args...)
    ...
    decltype(auto) towards_sub_oh_regular(
        ...
        return is_full
                   ? (is_leaf ? make_full_leaf_pos(child).visit(v, idx - lsize, args...)
                              : make_full_pos(child, p.shift() - B).visit(v, idx - lsize, args...))
                   : (is_leaf
                          ? make_leaf_sub_pos(child, size - lsize).visit(v, idx - lsize, args...)
    ...
    struct regular_sub_pos {
        ...
        auto size_sbh(count_t offset, size_t size_before_hint) {
            ...
            return offset == subindex(size_ - 1) ? size_ - size_before_hint : size_t{1} << shift_;
        }
        ...
        void copy_sizes(count_t offset, count_t n, size_t init, size_t* sizes) {
            if (n) {
                ...
                auto e = sizes + n - 1;
                for (; sizes != e; ++sizes) init = *sizes = init + (size_t{1} << shift_);
        ...
        decltype(auto) first_sub(Visitor v, Args&&... args) {
            ...
            return is_full
                       ? (is_leaf ? make_full_leaf_pos(child).visit(v, args...)
...
#pragma GCC diagnostic pop
    ...
    struct full_pos {
        ...
        decltype(auto) towards_oh(Visitor v, size_t idx, count_t offset_hint, Args&&... args) {
            ...
            return is_leaf ? make_full_leaf_pos(child).visit(v, idx, args...)
        ...
        decltype(auto) towards_sub_oh(Visitor v, size_t idx, count_t offset_hint, Args&&... args) {
            ...
            return is_leaf ? make_full_leaf_pos(child).visit(v, idx - lsize, args...)
    ...
    struct relaxed_pos {
        ...
        size_t size(count_t offset) const { return size_sbh(offset, size_before(offset)); }
        ...
        bool each_pred(Visitor v, Args&&... args) {
            ...
            if (shift_ == BL) {
                for (auto i = count_t{0}; i < n; ++i) {
                    ...
                    if (!make_leaf_sub_pos(p[i], relaxed_->d.sizes[i] - s).visit(v, args...))
                        return false;
                ...
                auto ss = shift_ - B;
                for (auto i = count_t{0}; i < n; ++i) {
                    if (!visit_maybe_relaxed_sub(p[i], ss, relaxed_->d.sizes[i] - s, v, args...))
                        return false;
                    s = relaxed_->d.sizes[i];
                }
            }
        ...
        void each_i(Visitor v, count_t i, count_t n, Args&&... args) {
            if (shift_ == BL) {
                ...
                for (; i < n; ++i) {
                    IMMER_PREFETCH(p + i + 1);
                    make_leaf_sub_pos(p[i], relaxed_->d.sizes[i] - s).visit(v, args...);
                    s = relaxed_->d.sizes[i];
                }
            } else {
                auto p = node_->inner();
                ...
                auto ss = shift_ - B;
                for (; i < n; ++i) {
                    visit_maybe_relaxed_sub(p[i], ss, relaxed_->d.sizes[i] - s, v, args...);
                    s = relaxed_->d.sizes[i];
                }
            }
        }
        ...
        template <typename Visitor, typename... Args>
        decltype(auto) towards_oh_sbh(
            Visitor v, size_t idx, count_t offset_hint, size_t left_size_hint, Args&&... args) {
        ...
        template <typename Visitor, typename... Args>
        decltype(auto) towards_sub_oh_sbh(
            Visitor v, size_t idx, count_t offset_hint, size_t left_size_hint, Args&&... args) {
            assert(offset_hint == index(idx));
            assert(left_size_hint == (offset_hint ? relaxed_->d.sizes[offset_hint - 1] : 0));
            auto child = node_->inner()[offset_hint];
            auto is_leaf = shift_ == BL;
            auto next_size = relaxed_->d.sizes[offset_hint] - left_size_hint;
            auto next_idx = idx - left_size_hint;
            return is_leaf ? make_leaf_sub_pos(child, next_size).visit(v, next_idx, args...)
                           : visit_maybe_relaxed_sub(child, shift_ - B, next_size, v, next_idx,
                                                     args...);
        }
        ...
        decltype(auto) last_oh_csh(Visitor v,
            ...
            return is_leaf
                       ...
                       : visit_maybe_relaxed_sub(child, shift_ - B, child_size_hint, v, args...);
        }
    ...
    template <typename Pos>
    using is_relaxed =
        std::is_same<relaxed_pos<typename std::decay_t<Pos>::node_t>, std::decay_t<Pos>>;
    ...
    decltype(auto) visit_maybe_relaxed_sub(
        ...
        if (relaxed) {
            ...
            return make_relaxed_pos(node, shift, relaxed).visit(v, std::forward<Args>(args)...);
        } else {
            return make_regular_sub_pos(node, shift, size).visit(v, std::forward<Args>(args)...);
        }
    }
...
#pragma GCC diagnostic pop
        ...
        decltype(auto) descend(Visitor v, size_t idx) {
            ...
            return r ? relaxed_descent_pos<NodeT, Shift - B>{child, r}.visit(v, next_idx)
