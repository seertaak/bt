...
namespace immer { namespace detail { namespace rbts {
    ...
    struct rbtree {
        ...
        static const rbtree& empty() {
            static const rbtree empty_{0, BL, node_t::make_inner_n(0u), node_t::make_leaf_n(0u)};
            return empty_;
        }
        ...
        rbtree(const rbtree& other) : rbtree{other.size, other.shift, other.root, other.tail} {
            inc();
        }
        ...
        bool traverse_p(Visitor v, Args&&... args) const {
            auto tail_off = tail_offset();
            auto tail_size = size - tail_off;
            return (tail_off ? make_regular_sub_pos(root, shift, tail_off).visit(v, args...)
                             : make_empty_regular_pos(root).visit(v, args...)) &&
                   make_leaf_sub_pos(tail, tail_size).visit(v, args...);
        }
        ...
        bool equals(const rbtree& other) const {
            ...
            return (size <= branches<BL> || make_regular_sub_pos(root, shift, tail_offset())
        ...
        const T* array_for(size_t index) const { return descend(array_for_visitor<T>(), index); }
