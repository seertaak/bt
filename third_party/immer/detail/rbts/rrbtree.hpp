...
namespace immer { namespace detail { namespace rbts {
    ...
    struct rrbtree {
        ...
        static const rrbtree& empty() {
            static const rrbtree empty_{0, BL, node_t::make_inner_n(0u), node_t::make_leaf_n(0u)};
            return empty_;
        }
        ...
        rrbtree(const rrbtree& other) : rrbtree{other.size, other.shift, other.root, other.tail} {
            inc();
        }
        ...
        void traverse(Visitor v, size_t first, size_t last, Args&&... args) const {
            ...
            if (last > tail_off)
                make_leaf_sub_pos(tail, tail_size)
                    .visit(v, first > tail_off ? first - tail_off : 0, last - tail_off, args...);
        }
        ...
        bool traverse_p(Visitor v, Args&&... args) const {
            auto tail_off = tail_offset();
            auto tail_size = size - tail_off;
            return (tail_off ? visit_maybe_relaxed_sub(root, shift, tail_off, v, args...)
                             : make_empty_regular_pos(root).visit(v, args...)) &&
        ...
        bool traverse_p(Visitor v, size_t first, size_t last, Args&&... args) const {
            ...
            return (first < tail_off
                        ? visit_maybe_relaxed_sub(root, shift, tail_off, v, first,
                                                  last < tail_off ? last : tail_off, args...)
                        : true) &&
                   (last > tail_off ? make_leaf_sub_pos(tail, tail_size)
        ...
        decltype(auto) descend(Visitor v, size_t idx) const {
            auto tail_off = tail_offset();
            return idx >= tail_off ? make_leaf_descent_pos(tail).visit(v, idx - tail_off)
        ...
        bool equals(const rrbtree& other) const {
            ...
            if (tail_off > 0 && tail_off_other > 0) {
                ...
                if (other.shift >= shift) {
                    if (!visit_maybe_relaxed_sub(other.root, other.shift, tail_off_other,
                                                 equals_visitor::rrb{}, iter_t{other}, root, shift,
                                                 tail_off))
                        return false;
                } else {
                    if (!visit_maybe_relaxed_sub(root, shift, tail_off, equals_visitor::rrb{},
                                                 iter_t{*this}, other.root, other.shift,
                                                 tail_off_other))
                        return false;
                }
            }
            return tail_off == tail_off_other
                       ? make_leaf_sub_pos(tail, tail_size()).visit(equals_visitor{}, other.tail)
                       : tail_off > tail_off_other
                             ? std::equal(tail->leaf(), tail->leaf() + (size - tail_off),
        ...
        std::tuple<shift_t, node_t*> push_tail(
            node_t* root, shift_t shift, size_t size, node_t* tail, count_t tail_size) const {
        ...
        rrbtree push_back(T value) const {
            auto ts = tail_size();
            if (ts < branches<BL>) {
                auto new_tail = node_t::copy_leaf_emplace(tail, ts, std::move(value));
                ...
                auto tail_off = tail_offset();
                try {
                    auto new_root = push_tail(root, shift, tail_off, tail, size - tail_off);
                    tail->inc();
        ...
        std::tuple<const T*, size_t, size_t> region_for(size_t idx) const {
            ...
            if (idx >= tail_off) {
                ...
                auto subs =
                    visit_maybe_relaxed_sub(root, shift, tail_off, region_for_visitor<T>(), idx);
        ...
        T& get_mut(edit_t e, size_t idx) {
            ...
            if (idx >= tail_off) {
                ...
                return visit_maybe_relaxed_sub(root, shift, tail_off, get_mut_visitor<node_t>{},
                                               idx, e, &root);
            }
        }

        const T& get(size_t index) const { return descend(get_visitor<T>(), index); }
        ...
        rrbtree update(size_t idx, FnT&& fn) const {
            ...
            if (idx >= tail_off) {
                ...
                auto new_root = visit_maybe_relaxed_sub(root, shift, tail_off,
        ...
        void drop_mut(edit_t e, size_t elems) {
            ...
            if (elems == 0) {
                ...
                *this = empty();
                ...
                tail =
                    get<1>(make_leaf_sub_pos(tail, size - tail_off).visit(v, elems - tail_off, e));
                if (root != empty().root) {
                    dec_inner(root, shift, tail_off);
                    shift = BL;
                    root = empty().root->inc();
        ...
        rrbtree drop(size_t elems) const {
            if (elems == 0) {
                ...
                auto tail_off = tail_offset();
                auto new_tail = node_t::copy_leaf(tail, elems - tail_off, size - tail_off);
        ...
        rrbtree concat(const rrbtree& r) const {
            ...
            if (size == 0)
                ...
                if (tail_size == branches<BL>) {
                    auto new_root = push_tail(root, shift, tail_offst, tail, tail_size);
                    tail->inc();
                    return {size + r.size, get<0>(new_root), get<1>(new_root), r.tail->inc()};
                } else if (tail_size + r.size <= branches<BL>) {
                    auto new_tail = node_t::copy_leaf(tail, tail_size, r.tail, r.size);
                    return {size + r.size, shift, root->inc(), new_tail};
                } else {
                    auto remaining = branches<BL> - tail_size;
                    auto add_tail = node_t::copy_leaf(tail, tail_size, r.tail, remaining);
                    try {
                        auto new_tail = node_t::copy_leaf(r.tail, remaining, r.size);
                        try {
                            auto new_root =
                                push_tail(root, shift, tail_offst, add_tail, branches<BL>);
                ...
                auto tail_offst = tail_offset();
                auto tail_size = size - tail_offst;
                auto concated = concat_trees(tail, tail_size, r.root, r.shift, r.tail_offset());
                ...
                auto tail_offst = tail_offset();
                auto tail_size = size - tail_offst;
                auto concated = concat_trees(root, shift, tail_offst, tail, tail_size, r.root,
        ...
        constexpr static bool supports_transient_concat = !std::is_empty<edit_t>::value;
        ...
        friend void concat_mut_l(rrbtree& l, edit_t el, const rrbtree& r) {
            ...
            if (l.size == 0)
                ...
                if (tail_size == branches<BL>) {
                    ...
                    auto remaining = branches<BL> - tail_size;
                    l.ensure_mutable_tail(el, tail_size);
                    std::uninitialized_copy(r.tail->leaf(), r.tail->leaf() + remaining,
                                            l.tail->leaf() + tail_size);
                    try {
                ...
                if (supports_transient_concat) {
                    auto tail_offst = l.tail_offset();
                    auto tail_size = l.size - tail_offst;
                    auto concated = concat_trees_mut(el, el, l.tail, tail_size,
                    ...
                    auto tail_size = l.size - tail_offst;
                    auto concated =
                        concat_trees(l.tail, tail_size, r.root, r.shift, r.tail_offset());
                    l = {l.size + r.size, concated.shift(), concated.node(), r.tail->inc()};
                    return;
                }
            } else {
                if (supports_transient_concat) {
                    auto tail_offst = l.tail_offset();
                    auto tail_size = l.size - tail_offst;
                    auto concated = concat_trees_mut(el, el, l.root, l.shift, tail_offst, l.tail,
                                                     tail_size, MemoryPolicy::transience_t::noone,
                    ...
                    auto tail_offst = l.tail_offset();
                    auto tail_size = l.size - tail_offst;
                    auto concated = concat_trees(l.root, l.shift, tail_offst, l.tail, tail_size,
                                                 r.root, r.shift, r.tail_offset());
                    l = {l.size + r.size, concated.shift(), concated.node(), r.tail->inc()};
                }
            }
        ...
        friend void concat_mut_r(const rrbtree& l, rrbtree& r, edit_t er) {
            ...
            if (r.size == 0)
                ...
                if (tail_size == branches<BL>) {
                    ...
                    // are tagged with `er`
                    auto res = l.push_tail(l.root, l.shift, tail_offst, l.tail, tail_size);
                    ...
                    auto new_tail = node_t::copy_leaf(l.tail, tail_size, r.tail, r.size);
                    r = {l.size + r.size, l.shift, l.root->inc(), new_tail};
                    return;
                } else {
                    // like the immutable version
                    auto remaining = branches<BL> - tail_size;
                    auto add_tail = node_t::copy_leaf_e(er, l.tail, tail_size, r.tail, remaining);
                    try {
                        ...
                        try {
                            ...
                            // are tagged with `er`
                            auto new_root =
                                l.push_tail(l.root, l.shift, tail_offst, add_tail, branches<BL>);
                            r = {l.size + r.size, get<0>(new_root), get<1>(new_root), new_tail};
                            return;
                        } catch (...) {
                            node_t::delete_leaf(new_tail, r.size - remaining);
                            throw;
                ...
                if (supports_transient_concat) {
                    ...
                    auto tail_size = l.size - tail_offst;
                    auto concated =
                        concat_trees_mut(er, MemoryPolicy::transience_t::noone, l.tail, tail_size,
                    ...
                    auto concated =
                        concat_trees(l.tail, tail_size, r.root, r.shift, r.tail_offset());
                    r = {l.size + r.size, concated.shift(), concated.node(), r.tail->inc()};
                    return;
                }
            } else {
                if (supports_transient_concat) {
                    auto tail_offst = l.tail_offset();
                    auto tail_size = l.size - tail_offst;
                    auto concated = concat_trees_mut(er, MemoryPolicy::transience_t::noone, l.root,
                                                     l.shift, tail_offst, l.tail, tail_size, er,
                                                     r.root, r.shift, r.tail_offset());
                    IMMER_ASSERT_TAGGED(concated.shift() == concated.node()->compute_shift());
                    assert(concated.node()->check(concated.shift(), l.size + r.tail_offset()));
                    r.size += l.size;
            ...
            if (l.size == 0)
                ...
                if (tail_size == branches<BL>) {
                    ...
                    if (r.tail->can_mutate(er))
                        detail::uninitialized_move(r.tail->leaf(), r.tail->leaf() + r.size,
                                                   l.tail->leaf() + tail_size);
                    else
                        std::uninitialized_copy(r.tail->leaf(), r.tail->leaf() + r.size,
                                                l.tail->leaf() + tail_size);
                    ...
                    l.ensure_mutable_tail(el, tail_size);
                    if (r.tail->can_mutate(er))
                        detail::uninitialized_move(r.tail->leaf(), r.tail->leaf() + remaining,
                                                   l.tail->leaf() + tail_size);
                    else
                        std::uninitialized_copy(r.tail->leaf(), r.tail->leaf() + remaining,
                                                l.tail->leaf() + tail_size);
                ...
                if (supports_transient_concat) {
                    auto tail_offst = l.tail_offset();
                    auto tail_size = l.size - tail_offst;
                    auto concated = concat_trees_mut(el, el, l.tail, tail_size, er, r.root, r.shift,
                                                     r.tail_offset());
                    IMMER_ASSERT_TAGGED(concated.shift() == concated.node()->compute_shift());
                    assert(concated.node()->check(concated.shift(), l.size + r.tail_offset()));
                    l.size += r.size;
                ...
                if (supports_transient_concat) {
                    ...
                    auto tail_size = l.size - tail_offst;
                    auto concated =
                        concat_trees_mut(el, el, l.root, l.shift, tail_offst, l.tail, tail_size, er,
                                         r.root, r.shift, r.tail_offset());
                    IMMER_ASSERT_TAGGED(concated.shift() == concated.node()->compute_shift());
                    assert(concated.node()->check(concated.shift(), l.size + r.tail_offset()));
                    l.size += r.size;
        ...
        friend void concat_mut_lr_r(rrbtree& l, edit_t el, rrbtree& r, edit_t er) {
            ...
            if (r.size == 0)
                ...
                if (supports_transient_concat) {
                    auto tail_offst = l.tail_offset();
                    auto tail_size = l.size - tail_offst;
                    auto concated = concat_trees_mut(er, el, l.tail, tail_size, er, r.root, r.shift,
                                                     r.tail_offset());
                    IMMER_ASSERT_TAGGED(concated.shift() == concated.node()->compute_shift());
                    assert(concated.node()->check(concated.shift(), l.size + r.tail_offset()));
                    r.size += l.size;
                ...
                if (supports_transient_concat) {
                    ...
                    auto tail_size = l.size - tail_offst;
                    auto concated =
                        concat_trees_mut(er, el, l.root, l.shift, tail_offst, l.tail, tail_size, er,
                                         r.root, r.shift, r.tail_offset());
                    IMMER_ASSERT_TAGGED(concated.shift() == concated.node()->compute_shift());
                    assert(concated.node()->check(concated.shift(), l.size + r.tail_offset()));
                    r.size += l.size;
        ...
        void debug_print_node(std::ostream& out,
            ...
            if (shift == endshift<B, BL>) {
                ...
                out << "- {" << size << "} " << pretty_print_array(node->leaf(), size) << std::endl;
            } else if (auto r = node->relaxed()) {
                auto count = r->d.count;
                debug_print_indent(out, indent);
                out << "# {" << size << "} " << pretty_print_array(r->d.sizes, r->d.count)
                    << std::endl;
                auto last_size = size_t{};
                for (auto i = count_t{}; i < count; ++i) {
                    debug_print_node(out, node->inner()[i], shift - B, r->d.sizes[i] - last_size,
                                     indent + indent_step);
                ...
                debug_print_indent(out, indent);
                out << "+ {" << size << "}" << std::endl;
                auto count = (size >> shift) + (size - ((size >> shift) << shift) > 0);
                if (count) {
                    for (auto i = count_t{}; i < count - 1; ++i)
                        debug_print_node(out, node->inner()[i], shift - B, 1 << shift,
                                         indent + indent_step);
                    debug_print_node(out, node->inner()[count - 1], shift - B,
                                     size - ((count - 1) << shift), indent + indent_step);
                }
            }
        }
