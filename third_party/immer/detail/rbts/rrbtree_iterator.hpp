...
namespace immer { namespace detail { namespace rbts {
    ...
    struct rrbtree_iterator : iterator_facade<rrbtree_iterator<T, MP, B, BL>,
        ...
        rrbtree_iterator(const tree_t& v) : v_{&v}, i_{0}, curr_{nullptr, ~size_t{}, ~size_t{}} {}
    ...
    private:
        ...
        const T& dereference() const {
            using std::get;
            if (i_ < get<1>(curr_) || i_ >= get<2>(curr_)) curr_ = v_->region_for(i_);
