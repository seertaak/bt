...
namespace immer { namespace detail {
    ...
    auto uninitialized_move(Iter1 in1, Iter1 in2, Iter2 out) {
        return std::uninitialized_copy(std::make_move_iterator(in1), std::make_move_iterator(in2),
                                       out);
    }
...
// inline auto clz_(unsigned __int64 x) { return __lzcnt64(x); }
    ...
    template <typename Iterator,
              typename Sentinel,
              std::enable_if_t<detail::std_distance_supports_v<Iterator, Sentinel>, bool> = true>
    typename std::iterator_traits<Iterator>::difference_type distance(Iterator first,
                                                                      Sentinel last) {
    ...
    template <typename Iterator,
              typename Sentinel,
              std::enable_if_t<(!detail::std_distance_supports_v<Iterator, Sentinel>)&&detail::
    ...
    template <typename SourceIter,
              ...
              typename SinkIter,
              std::enable_if_t<
                  (!detail::std_uninitialized_copy_supports_v<SourceIter, Sent, SinkIter>)&&detail::
