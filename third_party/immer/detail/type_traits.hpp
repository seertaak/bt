...
namespace immer { namespace detail {
    ...
    template <typename T, typename U = T>
    constexpr bool is_inequality_comparable_v = is_inequality_comparable<T, U>::value;
    ...
    struct is_iterator<T,
                       void_t<std::enable_if_t<is_preincrementable_v<T> &&
    ...
    struct is_forward_iterator<
        T,
        std::enable_if_t<is_iterator_v<T> && std::is_base_of<std::forward_iterator_tag,
    ...
    struct std_distance_supports<
        T,
        U,
        void_t<decltype(std::distance(std::declval<T>(), std::declval<U>()))>> : std::true_type {};
    ...
    struct std_uninitialized_copy_supports<
        ...
        void_t<decltype(std::uninitialized_copy(
            std::declval<T>(), std::declval<U>(), std::declval<V>()))>> : std::true_type {};
