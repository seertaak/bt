...
namespace immer { namespace detail { namespace arrays {
    ...
    struct node {
        ...
        constexpr static std::size_t sizeof_n(size_t count) {
            return immer_offsetof(impl_t, d.buffer) + sizeof(T) * (count == 0 ? 1 : count);
        }
        ...
        bool can_mutate(edit_t e) const { return refs().unique() || ownee().can_mutate(e); }
        ...
        static node_t* make_n(size_t n) { return new (heap::allocate(sizeof_n(n))) node_t{}; }
