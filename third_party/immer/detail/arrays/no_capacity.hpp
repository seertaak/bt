...
namespace immer { namespace detail { namespace arrays {
    ...
    struct no_capacity {
        ...
        no_capacity(const no_capacity& other) : no_capacity{other.ptr, other.size} { inc(); }

        no_capacity(no_capacity&& other) : no_capacity{empty()} { swap(*this, other); }
        ...
        T* data_mut(edit_t e) {
            if (!ptr->can_mutate(e)) ptr = node_t::copy_e(e, size, ptr, size);
            return data();
        }

        template <typename Iter,
                  typename Sent,
                  std::enable_if_t<is_forward_iterator_v<Iter> && compatible_sentinel_v<Iter, Sent>,
                                   bool> = true>
        ...
        static no_capacity from_fill(size_t n, T v) { return {node_t::fill_n(n, v), n}; }
        ...
        bool equals(const no_capacity& other) const {
            return ptr == other.ptr ||
                   (size == other.size && std::equal(data(), data() + size, other.data()));
        }
