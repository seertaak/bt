    ...
    struct champ {
        ...
        void for_each_chunk_traversal(node_t* node, count_t depth, Fn&& fn) const {
            if (depth < max_depth<B>) {
                ...
                if (nodemap) {
                    ...
                    for (; fst != lst; ++fst) for_each_chunk_traversal(*fst, depth + 1, fn);
                }
            } else {
        ...
        std::pair<node_t*, bool> do_add(node_t* node, T v, hash_t hash, shift_t shift) const {
            if (shift == max_shift<B>) {
                ...
                for (; fst != lst; ++fst)
                    if (Equal{}(*fst, v))
                        return {node_t::copy_collision_replace(node, fst, std::move(v)), false};
                ...
                if (node->nodemap() & bit) {
                    auto offset = popcount(node->nodemap() & (bit - 1));
                    auto result = do_add(node->children()[offset], std::move(v), hash, shift + B);
                    try {
                        result.first = node_t::copy_inner_replace(node, offset, result.first);
                        return result;
                    } catch (...) {
                    ...
                    auto val = node->values() + offset;
                    if (Equal{}(*val, v))
                        return {node_t::copy_inner_replace_value(node, offset, std::move(v)),
                                false};
                    else {
                        auto child =
                            node_t::make_merged(shift + B, std::move(v), hash, *val, Hash{}(*val));
                        try {
                            return {node_t::copy_inner_replace_merged(node, bit, offset, child),
                                    true};
                        } catch (...) {
                    ...
                    return {node_t::copy_inner_insert_value(node, bit, std::move(v)), true};
                }
            }
        }
        ...
        std::pair<node_t*, bool> do_update(
            ...
            if (shift == max_shift<B>) {
                ...
                return {node_t::copy_collision_insert(
                            node, Combine{}(std::forward<K>(k), std::forward<Fn>(fn)(Default{}()))),
                        true};
            } else {
                auto idx = (hash & (mask<B> << shift)) >> shift;
                auto bit = bitmap_t{1u} << idx;
                if (node->nodemap() & bit) {
                    ...
                    auto result = do_update<Project, Default, Combine>(
                        node->children()[offset], k, std::forward<Fn>(fn), hash, shift + B);
                    try {
                        result.first = node_t::copy_inner_replace(node, offset, result.first);
                        return result;
                    } catch (...) {
                    ...
                    if (Equal{}(*val, k))
                        ...
                        auto child = node_t::make_merged(
                            shift + B,
                            Combine{}(std::forward<K>(k), std::forward<Fn>(fn)(Default{}())), hash,
                            *val, Hash{}(*val));
                        try {
                            return {node_t::copy_inner_replace_merged(node, bit, offset, child),
                                    true};
                    ...
                    return {node_t::copy_inner_insert_value(
                                node, bit,
                                Combine{}(std::forward<K>(k), std::forward<Fn>(fn)(Default{}()))),
                            true};
                }
            }
        }
        ...
        champ update(const K& k, Fn&& fn) const {
            auto hash = Hash{}(k);
            auto res = do_update<Project, Default, Combine>(root, k, std::forward<Fn>(fn), hash, 0);
            auto new_size = size + (res.second ? 1 : 0);
            return {res.first, new_size};
        }
        ...
        sub_result do_sub(node_t* node, const K& k, hash_t hash, shift_t shift) const {
            if (shift == max_shift<B>) {
                ...
                if (node->nodemap() & bit) {
                    auto offset = popcount(node->nodemap() & (bit - 1));
                    auto result = do_sub(node->children()[offset], k, hash, shift + B);
                    switch (result.kind) {
                    case sub_result::nothing: return {};
                    case sub_result::singleton:
                        return node->datamap() == 0 && popcount(node->nodemap()) == 1 && shift > 0
                                   ? result
                                   : node_t::copy_inner_replace_inline(node, bit, offset,
                                                                       *result.data.singleton);
                    case sub_result::tree:
                        try {
                            return node_t::copy_inner_replace(node, offset, result.data.tree);
                        } catch (...) {
                    ...
                    if (Equal{}(*val, k)) {
                        ...
                        if (node->nodemap() || nv > 2)
                            ...
                            return shift > 0 ? sub_result{node->values() + !offset}
                                             : node_t::make_inner_n(0, node->datamap() & ~bit,
                                                                    node->values()[!offset]);
                        } else {
                            assert(shift == 0);
        ...
        champ sub(const K& k) const {
            ...
            switch (res.kind) {
            case sub_result::nothing: return *this;
            case sub_result::tree: return {res.data.tree, size - 1};
        ...
        static bool equals_tree(const node_t* a, const node_t* b, count_t depth) {
            if (a == b)
                ...
                if (a->nodemap() != b->nodemap() || a->datamap() != b->datamap()) return false;
