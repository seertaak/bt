...
namespace immer { namespace detail { namespace hamts {
    ...
    struct champ_iterator : iterator_facade<champ_iterator<T, Hash, Eq, MP, B>,
        ...
        champ_iterator(const tree_t& v) : depth_{0} {
            if (v.root->datamap()) {
                cur_ = v.root->values();
        ...
        champ_iterator(const tree_t& v, end_t) : cur_{nullptr}, end_{nullptr}, depth_{0} {
            path_[0] = &v.root;
        }
