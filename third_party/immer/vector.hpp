...
template <typename T, typename MemoryPolicy, detail::rbts::bits_t B, detail::rbts::bits_t BL>
class flex_vector;
...
public:
     ...
     */
    vector(std::initializer_list<T> values) : impl_{impl_t::from_initializer_list(values)} {}
     ...
     */
    IMMER_NODISCARD bool operator==(const vector& other) const { return impl_.equals(other.impl_); }
    IMMER_NODISCARD bool operator!=(const vector& other) const { return !(*this == other); }
     ...
     * @endrst
     */
    IMMER_NODISCARD vector take(size_type elems) const& { return impl_.take(elems); }
...
#if IMMER_DEBUG_PRINT
    void debug_print(std::ostream& out = std::cerr) const { flex_t{*this}.debug_print(out); }
#endif
...
private:
    ...
    vector take_move(std::false_type, size_type elems) { return impl_.take(elems); }
