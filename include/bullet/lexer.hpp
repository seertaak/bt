        ...
        private:
            ...
            auto eat_numeric_literal(uint32_t& pos) -> bool {
                ...
                if (c == 'e' || c == 'E') {
                    ...
                    switch (base) {
                    case 10:
                        match = eat_digits<10, unsigned long long>(p, b);
                        break;
                    case 16:
                        match = eat_digits<16, unsigned long long>(p, b);
                        break;
                    case 8:
                        match = eat_digits<8, unsigned long long>(p, b);
                        break;
                    case 2:
                        match = eat_digits<8, unsigned long long>(p, b);
                        break;
                    ...
                    if (c == 'f') {
                        ...
                        s = remaining_input(++p);
                        if (s.size() < 2) 
                            throw std::runtime_error("Fucking bad floating");
                        s = s.substr(0, 2);
                        if (s == "32"sv) {
                            width = 32;
                            p += 2;
                    ...
                    if (!signedness) {
                        ...
                        if (p >= input_length)
                            throw std::runtime_error("Fuckinng bad int.");
                    ...
                    const auto last_col = pos - start_of_line.back() - 1;
                    tokens.emplace_back(
                        integral_t(intval, signedness, width), 
                        line, start_col, last_col);
                }
                return true;
            }
