...
namespace bt { namespace parser { namespace syntax {
    ...
    struct fn_closure_param_t {
        BOOST_HANA_DEFINE_STRUCT(fn_closure_param_t,
                                 (bool, var),
                                 (std::optional<lexer::with_loc<lexer::identifier_t>>, identifier),
    ...
    auto operator<<(std::ostream& os, const fn_closure_param_t<Attr>& p) -> std::ostream& {
        if (auto v = p.var) os << "var ";
        if (auto i = p.identifier) os << i->name;
        if (p.expression.get()) os << "=" << p.expression;
        return os;
    }
    ...
    struct fn_expr_t {
        BOOST_HANA_DEFINE_STRUCT(fn_expr_t,
                                 (std::vector<lexer::with_loc<lexer::identifier_t>>, arg_names),
    ...
    auto operator<<(std::ostream& os, const fn_expr_t<Attr>& a) -> std::ostream& {
        ...
        os << ")";
        if (a.result_type.get()) os << " -> " << a.result_type;
