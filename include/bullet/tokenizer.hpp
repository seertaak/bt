    ...
    auto tokens1(string_view input) -> vector<token_t> {
        ...
        const auto on_margin_end = [&margins, &colon_indent, &b_ws, &pending_dedents, input](auto& ctx) {
            ...
            if (n == margin) {
                ...
                if (b_ws != begin(input))
                    _val(ctx).emplace_back(SEMICOLON);
            } else if (n > margin) {
        ...
        auto line_end = x3::rule<class identifier_type, string>("line_end") =
            lit("\r\n") | lit('\r') | lit('\n');
        ...
        const auto print_hello = [=] (auto ctx) {
            cout << (begin(_where(ctx)) - begin(input)) << endl;
        };
        ...
        const auto lines = *(empty_line | non_empty_line);
            /*
            line >> *(
                 -lit(':')[on_colon] >> x3::eol >> line
            );
            */

            //line % (-(lit(':') [on_colon]) >> x3::eol);

            //line % ( -(lit(':') [on_colon]) >> x3::eol >> attr(SEMICOLON));
